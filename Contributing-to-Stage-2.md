...
The CBE is being worked on by multiple people, please see [what tests are taken](https://github.com/ziglang/zig/wiki/C-Backend-Behavioral-Tests) before working on one, and mark yourself when tackling a new one. Unfortunately, a new passing test could make unrelated tests pass, which means that some duplicated work could happen anyway.
